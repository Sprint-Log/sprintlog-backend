...
def test_access_log_filter_status_ok() -> None:
    log_record = LogRecord(
        ...
        lineno=1,
        msg="message",
        args=("", settings.openapi.HEALTH_PATH, "", HTTP_200_OK),
        exc_info=None,
    )
    log_filter = logging.AccessLogFilter(path_re=settings.openapi.HEALTH_PATH)
...
def test_access_log_filter_status_not_ok() -> None:
    log_record = LogRecord(
        ...
        lineno=1,
        msg="message",
        args=("", settings.openapi.HEALTH_PATH, "", HTTP_503_SERVICE_UNAVAILABLE),
        exc_info=None,
    )
    log_filter = logging.AccessLogFilter(path_re=settings.openapi.HEALTH_PATH)
